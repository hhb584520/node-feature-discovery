	...
	Convey("When parsing command line arguments", t, func() {
		...
		Convey("When --no-publish and --oneshot flags are passed", func() {
			...
			Convey("noPublish is set and args.sources is set to the default value", func() {
				...
				So(args.sources, ShouldResemble, []string{"cpu", "cpuid", "iommu", "kernel", "local", "memory", "network", "os-release", "pstate", "rdt", "selinux", "storage"})
    ...
    configFlags:
      - "DMI"`)
		...
		Convey("When proper config file is given", func() {
			...
			Convey("Should return error", func() {
				So(err, ShouldBeNil)
				So(config.Sources.Kernel.ConfigFlags, ShouldContain, "DMI")
			})
		})
	})
}
